    ...
    var App = App || {
        init: function () {
            ...
            $('.js-search .text-input').keydown(function (event) {
                if (event.keyCode == 13) {
                    location.href = 'https://www.google.com/search?q=site:zhongzilu.github.io' + $(this).val();
                    return false;
                }
            })
        }
...
//            bannerNode.attr('style','background-image:url(/banner/'+index+'.jpg)');
            bannerNode.attr('style','background-image:url(http://ozpzmw1tj.bkt.clouddn.com/image/banner/'+index+'.jpg)');
        }
    })
